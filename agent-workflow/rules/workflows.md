...
同時Issue worktree数は、`git worktree list --porcelain`に実在し、命名規則
`<repository>-issue-{N}`に一致するすべてのworktreeを数えます。レジストリ未登録の実在worktreeも
上限を消費し、driftとして新規作成を停止します。`active`・`blocked_quota`・
`pending_cleanup_approval`はいずれも、worktreeが削除されるまで上限を消費します。
subagent数とは別の上限です。
