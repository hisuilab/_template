...
- [3. ファイルとディレクトリ名](#3-ファイルとディレクトリ名)
  - [3.1. Issue worktree](#31-issue-worktree)
  - [3.2. Worktreeレジストリ](#32-worktreeレジストリ)
- [4. 文書の状態管理](#4-文書の状態管理)
- [5. 例外](#5-例外)
...
Issue worktreeはprimary manager worktreeの兄弟へ`<repository>-issue-{N}`で作成します。
`<repository>`はprimary manager worktreeのbasenameから末尾の`-main`を除いた値です。末尾が
`-main`でない場合はbasenameをそのまま使います。

例: `/workspace/example-main`でIssue #12を開始する場合、pathは
`/workspace/example-issue-12`、branchは`type/issue-12-topic`です。作成前にresolveしたpathの
親がprimary manager worktreeの親と一致し、path・branch・Issue番号が未使用であることを
確認します。

### 3.2. Worktreeレジストリ

primary manager worktreeの`tmp/worktrees.json`を、Issueとworktree・担当ツールの関連付けの
正本とします。形式は次の通りです。
...
```json
{
  ...
  "worktrees": {
    "issue-12": {
      "path": "../example-issue-12",
      "branch": "feat/issue-12-profile-resolver",
      "issue": 12,
      "assigned_tool": "codex",
      "pr": null,
      "phase": "design",
...
- `version`は`1`のみを受け付け、未知のversionは書き換えず停止します
- entry keyは`issue-{N}`です。必須フィールドは例に示す8項目です
- `assigned_tool`は`claude` / `codex` / `human` / `null`です
- `pr`は正の整数または`null`、`phase`はフェーズ名または`null`です
- `status`は`active` / `blocked_quota` / `pending_cleanup_approval`です
- `updated_at`はタイムゾーン付きISO 8601日時です
- 実在は`git worktree list --porcelain`、phaseは各worktreeの`phase-state.json`、PR状態は
  GitHubを正本とし、レジストリの対応フィールドは最新観測値として扱います
- 更新は同じディレクトリの一時ファイルへ完全なJSONを書き、構文を検証してからrenameする
  原子的な置換で行います
- 読み取り後に更新する操作は、`mkdir tmp/worktrees.lock`の成功を排他lock取得とします。
  lock内の`owner.json`へ一意なowner token、host、PID、process開始時刻、operation、agent、
  取得日時を記録し、最新状態を読み直してから処理します
- lockは成功・失敗のどちらでも解放します。既存lockがあれば処理を開始せずownerを報告します。
  `scripts/worktree-safety lock-status`が同じhostのPIDとprocess開始時刻の一致から`live`・`stale`・
  `unknown`を判定します。`live`と`unknown`は回収せず、`stale`だけをowner token提示後のPM承認を
  得て`scripts/worktree-safety reclaim-lock`で回収します
- worktree作成・削除、local branch削除、担当変更、stale lock回収は
  `scripts/worktree-safety`の対応するplan/apply操作を必須経路とします。AIエージェントが同等の
  shell操作を独自に組み立てて代替してはいけません
