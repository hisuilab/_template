...
3. 選択された開始位置から未完了フェーズを順に実行します(各コマンドの規約に従います)
   ...
   > [!NOTE]
   > **後方互換**: `completed`に`investigate`がなく、かつ`completed`に他のフェーズ
   > (`design`等)が1つ以上ある既存の`phase-state.json`は「investigate未実施の継続中issue」
   > として扱い、スキップして次の未完了フェーズから継続します。`completed`が空かつ
   > `current`がnullの新規状態では`investigate`を先頭から実行します。
