...
| `lang` | `lang/python` / `lang/typescript` / `lang/rust` / `lang/go` | 言語環境（flake.nix replace・treefmt・justfile・.gitignore）。`lang/typescript`は共通`package.json`を含み、`lang/rust`・`lang/go`はWebAPI等の複合Partが積む基盤依存(tracing/serde/anyhow・godotenv等)もCargo.toml/go.modに含む |
...
> [!NOTE]
> 「実装済み」の状態は 2.節（現行実装）が正本です。現行実装との差分（`starter/web-htmx`・`lang/rust`・`lang/go`・複合 Part 等）はこの表には含まれていません。
