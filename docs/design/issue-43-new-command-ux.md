...
- [ ] `just new` → フルウィザード（現状と同じ動作）
- [ ] `just new myapp` → name 事前入力済みウィザード（lang/profile のみ選択）
- [ ] `just new myapp --lang python --profile small-cli` → 完全非対話で即生成
- [ ] `just new myapp --output ./myapp` → worktree 追加（`./myapp/myapp-api/` に生成）
- [ ] `generate` から `--name` を廃止し `--output` の意味を「親ディレクトリ」に統一
- [ ] 既存テストがすべて通る
- [ ] `just new test` がエラーにならない（Justfile のレシピ名誤解釈を解消）
...
```text
python3 -m tooling.generator create [--name NAME] [--lang LANG]
