...
| 決定論的なworktree safety helperとBatsテスト | GitHub APIアクセス、PM承認の代替、remote branch操作 |
...
| `scripts/worktree-safety`(新設) | registry・path・lock・上限・dirty・branch・PR head SHAを検証し、dry-run計画と承認後のlocal Git操作を原子的に実行する | GitHub状態の取得、GitHub Issue/PR/remote branch操作、PM承認 |
...
| 操作 | 入力 | 出力・副作用 |
| --- | --- | --- |
| `plan-create` | manager root・Issue・branch・path・tool | schema、親path、競合、未登録を含む実在Issue worktree数を検証し、base SHA付き作成計画を返します |
| `apply-create` | planと同じ入力・承認済みbase SHA | lock取得後に再検証し、worktree・phase-state・registryを一括作成します |
| `plan-cleanup` | manager root・Issue・PR state/head SHA | clean、branch、SHA、remote trackingを検証し、削除計画を返します |
| `apply-cleanup` | planと同じ入力・承認済みworktree SHA | lock取得後に再検証し、worktree・local branch・registry entryを一括削除します |
...
`plan:issue`と`manage:cleanup`は`plan-*`のJSONをPMへ提示します。承認後は対応する`apply-*`へ
承認済みSHAを渡します。`apply-*`プロセス自身がlockを保持し、状態再検証からlocal Git操作と
registry更新まで完了してから解放します。plan後に状態が変化すれば非0終了し、操作しません。
GitHub Issue作成は`apply-create`前、GitHub PR状態取得は`plan-cleanup`前にcommand側が行います。
...
| 8 | safety判定 | (a)proseだけで判定 / (b)helperがplanとapplyを担当しAIはGitHub状態と承認を仲介 | (b) | path・SHA・dirty・lock等の省略とplan/apply間競合を防ぎ、Batsで退行を検出できます。helperはlocal Gitだけを操作し、GitHub状態と承認の責任はcommand側に残します |
...
| Unit(Bats) | registry schema、lock liveness/reclaim、兄弟path、未登録worktreeを含む上限、dirty/detached、PR state/head SHA、remote tracking、assign、plan/apply間の状態変化、create/cleanupの部分失敗を一時Gitリポジトリで検証します |
