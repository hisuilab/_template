...
| squash merge後に`git branch -d`が失敗する | PRが`MERGED`、対象worktreeが削除済み、remoteにbranchが残っていることを確認してから、PMが承認したcleanup範囲内で`git branch -D`を許可します。条件を満たさなければ停止します |
...
| 同時実行数の上限超過 | `/plan:issue`のworktree作成時、`tmp/worktrees.json`の`status: active`件数が上限(3)に達していれば警告し、既存worktreeの完了を先に促す。強制的な追加作成は行わない |
...
| スキーマ | version・必須フィールド・列挙値・未知version時の停止・正本との同期規則が`naming-policy.md`とコマンド契約で一致することをレビューする |
| 手動統合検証 | 一時的なテスト用worktreeを作成し、登録・status同期・cleanup候補検出まで確認します。破壊操作は対象を明示して別途承認し、承認されない場合は削除直前までを確認します |
