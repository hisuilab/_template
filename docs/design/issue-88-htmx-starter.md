...
- **CDN読み込みのSRI・バージョン固定(レビューで指摘)**: `templates/layout.html`は
  `pico@2`・`htmx.org@2`とメジャーバージョンのみを指定し、`integrity`属性(Subresource
  Integrity)を付与していません。マイナー/パッチ更新で予期しない変化が起きるリスクと、
  CDN側の改ざんに対する検証手段が無いというsupply-chainリスクがあります。オフライン
  vendoring(上記項目)を実施する際に、固定バージョン+SRIハッシュもあわせて導入する
  想定とします
