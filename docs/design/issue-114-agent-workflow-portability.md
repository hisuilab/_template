...
- [4. 設計案](#4-設計案)
  ...
  - [4.2. 責務分離](#42-責務分離)
  - [4.3. 参照方式(Issue #115実装時に更新)](#43-参照方式issue-115実装時に更新)
...
- **Codex側**: 次の2点は実機検証が必要な未解決事項として残ります(U-01a/U-01b、7節参照)。
  ...
  2. **明示的read指示への追従**: パス発見が成立した前提で、`.codex/AGENTS.md`の
     「`agent-workflow/rules/`を読んでください」という指示にCodexが実際に従うか
     (これはClaude Code側と同じ「プローズで指示する」方式なので、`@file`構文サポートの
     有無に依存しません。ファイルを読んで指示に従う一般的な能力があれば成立する見込みです)
...
5. `scripts/check-readme`の要件(概要/責任/責任外)を満たすREADME.mdを、追跡対象になる
   ...
   `agents/`・`skills/*/`・`hooks/`)、および`.codex/`(`README.md`・`agents/`)です。
...
| ケース | 影響 | 対応 |
| --- | --- | --- |
| Codexが`.codex/AGENTS.md`を自動発見しない | `.codex/`adapterの入口が機能せず、Codex向け共通ワークフローの前提が崩れる | Issue #115で最優先に確認し、不成立の場合は`.codex/AGENTS.md`前提の設計を保留します。代替としてルート`AGENTS.md`への最小要約追記、またはCodex固有設定への本文埋め込みを比較する意思決定レポートを作成します |
...
| ID | 論点 | 決定者 | ブロックする作業 |
| --- | --- | --- | --- |
| U-01a | Codexがルート`AGENTS.md`とは別に`.codex/AGENTS.md`というパスを自動探索・読み込みするか(パス発見) | Issue #115(実機検証、最優先。`codex` CLIがこのセッションの環境に無く未検証) | Codexアダプター全体(4.1〜4.3)の前提 |
| U-01b | (U-01aが成立した場合)Codexが`.codex/AGENTS.md`の明示的read指示(「`agent-workflow/rules/`を読んでください」)に実際に従うか | Issue #115(実機検証) | 低リスク(`@file`構文非依存のプローズ指示のため、成立する見込みが高い) |
