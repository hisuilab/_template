...
- [4. 設計案](#4-設計案)
  ...
  - [4.2. 責務分離](#42-責務分離)
  - [4.3. 参照方式(Issue #125で更新)](#43-参照方式issue-125で更新)
...
```text
_template/
├── AGENTS.md                    # Claude/Codex共通入口。agent-workflow/への導線を持つ
...
| `.codex/` | Codex固有の実行機構(hooks等) | `agent-workflow/`内容の複製、Codex/Claude共通の入口 |
...
### 4.3. 参照方式(Issue #125で更新)

Issue #115の実装着手時、`@file`という自動展開構文への依存自体を見直しました。Web調査の結果、
`@file`import(`CLAUDE.md`が使うmemory file読み込み機構)と、スラッシュコマンド・subagent
定義ファイルでの`@file`参照は別の実装で、後者はAnthropic公式リポジトリでも
[未対応の機能要望(Issue #5914)](https://github.com/anthropics/claude-code/issues/5914)
として起票されている段階でした。動作しても「意図した動作ではない」挙動(キャッシュされず
毎回読み込む等)が報告されており、この構文に依存するのは高リスクと判断しました。

代わりに、**「このファイルを読んで、その指示に従ってください」という明示的なプローズ指示**を
採用しました。特別なimport機構に依存しない自然文の指示なので、Claude・Codex問わずファイルを
読んで指示に従うことができるどんなAIエージェントでも確実に動作します。

- **Claude Code側**: `.claude/commands/*.md`・`.claude/agents/*.md`・`.claude/skills/*/SKILL.md`
  は、権限ヘッダー(管理責任/ワークフロー/状態確認先/権限/副作用/承認、または
  frontmatter+権限ヘッダー)を残し、本文を`` `agent-workflow/...`を読み、その指示に従って
  ください。 ``という1行の指示に置き換えます。YAML frontmatter(`name`/`description`)は
  Claude Codeのsubagent/Skill発見に必要なため、`.claude/agents/*.md`・
  `.claude/skills/*/SKILL.md`側にそのまま残します
- **Codex側**: Issue #125の実機診断で、Codexはリポジトリルートの`AGENTS.md`を自動読込
  する一方、`.codex/AGENTS.md`は初期コンテキストへ入らないことを確認しました。したがって
  Codex固有の入口ファイルとして`.codex/AGENTS.md`を持つ設計は廃止し、Claude/Codex共通の
  入口をルート`AGENTS.md`へ集約します。ルート`AGENTS.md`は`agent-workflow/`が正本である
  ことと、`/auto:issue`等のワークフロー指定時に対応する`agent-workflow/commands/`を読む
  ことを明記します。`.codex/`はhooksなどCodex固有の実行機構のみを持ちます。
...
5. `scripts/check-readme`の要件(概要/責任/責任外)を満たすREADME.mdを、追跡対象になる
   すべての新規/復帰ディレクトリへ作成します。対象は`agent-workflow/`配下の新設ディレクトリ
   (`README.md`・`rules/`・`commands/`配下の各サブディレクトリ・`agents/`・`skills/*/`)、
   `.claude/`配下の追跡復帰ディレクトリ(`README.md`・`commands/`配下の各サブディレクトリ・
   `agents/`・`skills/*/`・`hooks/`)、および`.codex/`(`README.md`・`hooks/`)です。
   README.mdを置かないディレクトリが必要な場合は、Issue #115で理由を明示したうえで
   `.readmeignore`へ個別登録します
6. ルート`AGENTS.md`に、Claude/Codex共通入口として`agent-workflow/`への参照と
   ワークフローコマンドの読み替え規約を記述します。`.codex/AGENTS.md`はCodexの自動探索
   対象ではないため作成しません
7. `.gitignore`を4.4節の通りに書き換えます
8. ファイル移設・adapter薄化・README.md整備・`.gitignore`更新をそれぞれ検証可能な移行単位
   として扱い、各移行単位の完了時に`nix develop --command just verify`でグリーンを確認します
...
| ケース | 影響 | 対応 |
| --- | --- | --- |
| Codexが`.codex/AGENTS.md`を自動発見しない | `.codex/`adapterを入口にできない | Issue #125で発生を確認済みです。入口をルート`AGENTS.md`へ集約し、`.codex/`はCodex固有の実行機構だけを持つ構成へ補正しました |
| ルート`AGENTS.md`が肥大化する | Codexの`project_doc_max_bytes`上限に近づき、初期コンテキストが欠落する | 本文の複製は避け、`agent-workflow/`への導線とコマンド対応だけを記述します。詳細は対応する正本Markdownを必要時に読む方針にします |
...
| Codex入口 | `codex debug prompt-input`でルート`AGENTS.md`のワークフロー入口が初期コンテキストへ入ることを確認する |
...
U-01a/U-01bはIssue #125で解消済みです。Codexは`.codex/AGENTS.md`を入口として自動読込しない
ため、ルート`AGENTS.md`をClaude/Codex共通入口として使います。
