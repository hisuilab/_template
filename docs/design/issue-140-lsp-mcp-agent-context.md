...
- read-only 4ツール（definition・references・diagnostics・hover）は安全に利用できます
...
`features/ai-agent`のpayload `AGENTS.md`（生成プロジェクト向け）と、root `AGENTS.md`
（このリポジトリ向け）の両方にsemantic探索方針を追記します。
...
追記の具体的な内容は4.5節のfallback方針と合わせて実装フェーズで確定します。
...
本Issueの完了後に実施するtemlpate反映のfollow-up Issueを特定します。
