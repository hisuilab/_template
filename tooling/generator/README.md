...
- `planner.py`: 変数束縛・`dot-` プレフィックス除去・`{{変数}}` パス置換・衝突検出で `GenerationPlan` を生成（PLAN 段階）。変数は Profile variables を基底に予約語（`project_name`）で上書きする優先順位。`strategy="add"` は同名ファイルを先勝ちで維持
