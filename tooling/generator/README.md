...
- `models.py`: パイプライン共通データ型（`LangSpec` / `GenerateRequest` / `PlannedFile` / `GenerationPlan` / `GenerationResult` / `InjectResult` / `ManifestData` / `ManifestEntry`）
...
- `manifest.py`: `.template-manifest.toml` の読み書き（`write_manifest` / `read_manifest` / `update_manifest`）。schema_version 2 の v2 フォーマットを採用。`[[applied]]` エントリに `files`・`part_digest` を、`[manifest]` ヘッダに `template_revision`・`generator_version` を記録する。書き込みは `.tmp → rename` による atomic write。v1 ファイル読み込み時は `ManifestError` を上げ再生成を促す
- `services.py`: lang/profile の検証（`validate_lang` / `validate_profile`）と profile 拡張（`resolve_selection`）を CLI から独立した形で提供する。`generate` と `create` の双方から呼び出すことで同一 preflight を保証する（issue #132）
