...
- `models.py`: パイプライン共通データ型（`GenerateRequest` / `PlannedFile` / `GenerationPlan` / `GenerationResult`）
- `errors.py`: 各段階エラー型（`LoadError` / `ResolveError` / `PlanError` / `RenderError` / `ApplyError`）
